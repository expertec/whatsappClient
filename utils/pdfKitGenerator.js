 ...
 * @param {string|object} strategyText - Texto o JSON generado por ChatGPT con la estrategia.
...
export async function createStrategyPDF(strategyText, leadData) {
  return new Promise((resolve, reject) => {
    ...
    // Usamos la carpeta temporal /tmp (útil en entornos como Render)
    const outputPath = path.join('/tmp', fileName);
    // Creamos el documento PDF con un margen de 50
    ...
    doc.moveDown();
    // Convertir strategyText a cadena en caso de ser un objeto
    const strategyString =
      typeof strategyText === "string"
        ? strategyText
        : JSON.stringify(strategyText, null, 2);
    doc
      .fontSize(12)
      .text(strategyString, {
        align: "justify",
    ...
    // Generar un calendario para 4 semanas
    const weeks = 4;
    const days = ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"];
    for (let week = 1; week <= weeks; week++) {
      doc.fontSize(12).text(`Semana ${week}:`);
      doc.moveDown(0.5);
      days.forEach((day) => {
